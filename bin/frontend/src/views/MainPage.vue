<template>
  <div class="container">
    <h1 style="text-align: left; padding-top: 30px">자유 게시판</h1>
  ...
  methods: {
    async fetchData() {
      try {
        ...
        const {
          data: { data },
        } = await getboardList({ boardTypes: 'COMMUNITY', size: 1000 });
        this.isLoading = false;
        console.log(data);
        this.boardList = data;
      } catch (error) {
    ...
    gotoCreatePage() {
      this.$router.push({ path: '/create', query: { type: 'COMMUNITY' } });
    },
  },
  created() {
    this.fetchData();
